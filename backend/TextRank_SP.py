...
class MongolianTextRankSummarizer:
    ...
    def summarize(self, text, ratio):
        num_sentences = max(2, int(np.ceil(self.text_length * ratio)))
        ...
        sentence_scores = self.textrank(similarity_matrix)
        ...
        summary = ' '.join([processed_sentences[i][0] for i in selected_indices])
